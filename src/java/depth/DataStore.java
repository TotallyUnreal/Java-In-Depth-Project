...
import java.depth.constants.BookGenre;
import java.depth.constants.Gender;
import java.depth.constants.MovieGenre;
import java.depth.constants.UserType;
import java.depth.entities.Bookmark;
import java.depth.entities.User;
import java.depth.entities.UserBookmark;
import java.depth.managers.BookmarkService;
import java.depth.managers.UserService;
...
public class DataStore {
	...
	private static UserBookmark[] userBookmarks = new UserBookmark[BOOKMARK_TYPES_COUNT * USER_BOOKMARK_LIMIT];
	...
	private static void loadUsers() {
		users[0] = UserService.getInstance().createUser(1000, "user0@semanticsquare.com", "test", "John", "M",
				Gender.MALE, UserType.USER);
		users[1] = UserService.getInstance().createUser(1001, "user1@semanticsquare.com", "test", "Sam", "M",
				Gender.MALE, UserType.USER);
		users[2] = UserService.getInstance().createUser(1002, "user2@semanticsquare.com", "test", "Anita", "M",
				Gender.MALE, UserType.EDITOR);
		users[3] = UserService.getInstance().createUser(1003, "user3@semanticsquare.com", "test", "Sara", "M",
				Gender.FEMALE, UserType.EDITOR);
		users[4] = UserService.getInstance().createUser(1004, "user4@semanticsquare.com", "test", "Dheeru", "M",
